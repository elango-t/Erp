...
const Teacher = require('../models/professorSchema.js');
...
const subjectCreate = async (req, res) => {
    try {
        ...
        if (existingSubjectBySubCode) {
            res.send({ message: ' subject code must be unique ' });
        } else {
...
const classSubjects = async (req, res) => {
    try {
        let subjects = await Subject.find({ studentclassName: req.params.id })
        if (subjects.length > 0) {
            res.send(subjects)
        } else {
            res.send({ message: "No subjects found" });
...
const freeSubjectList = async (req, res) => {
    try {
        let subjects = await Subject.find({ studentclassName: req.params.id, teacher: { $exists: false } });
        if (subjects.length > 0) {
            res.send(subjects);
        } else {
            res.send({ message: "No subjects found" });
...
const getSubjectDetail = async (req, res) => {
    try {
        ...
        if (subject) {
            subject = await subject.populate("studentclassName", "studentclassName")
...
module.exports = { subjectCreate, freeSubjectList, classSubjects, getSubjectDetail,
     deleteSubjectsByClass, deleteSubjects, deleteSubject, allSubjects };